...
By this theorem, when the linear system is consistent and it has a unique solution, there are no free variables. What does this mean for the pivot columns? In order to answer this it will be easier to think about the structure of the matrix rows as well. A free variable can be identified when a matrix is in reduced row echelon form. Within a given row of a matrix in reduced row echelon form, if there are any coefficients to the right of that row's pivot variable (if there is a pivot variable in the row), then those coefficients are free variables.

\todo{Expand explanation with examples, and discuss how pivot position does not exist for a column corresponding to a free variable. Use examples from p 19 etc}

\todo{Add generalized argument using indices}
